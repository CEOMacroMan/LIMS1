...
export async function savePreserve() {
  ...
  try {
    ...
    logKV('[error]', { action: 'save', step: err.step || 'write', name: err.name, message: err.message });
...
export async function downloadPreserve() {
  ...
  try {
    ...
    logKV('[error]', { action: 'download', step: err.step || 'build-binary', name: err.name, message: err.message });
