...
export const fsApiAvailable = 'showOpenFilePicker' in window;
export const fsContextOk =
  location.protocol === 'https:' || ['localhost', '127.0.0.1'].includes(location.hostname);
export const fsSupported = fsApiAvailable && fsContextOk;
...
export async function openForEdit() {
  if (!fsSupported) {
    log(`[fs] unsupported ${!fsApiAvailable ? 'api' : 'context'}`);
    return;
  }
  let step = 'picker';
  try {
